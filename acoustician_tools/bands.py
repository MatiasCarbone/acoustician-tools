...
def third_octave_bands():
    """
    Generate a list of center and boundary frequencies for third-octave
    bands multiple of 1000Hz.
    Note - these are exact frequencies (rounded to 3 decimal), not
    standard (nominal) frequencies.
    ...
    ratio = 2 ** (1 / 3)
    while min(f0) > 16:
        f0.insert(0, f0[0] / ratio)
    while max(f0) < 20000:
        f0.append(f0[-1] * ratio)

    flow = np.asarray(f0) / 2 ** (1 / 6)
    fhigh = np.asarray(f0) * 2 ** (1 / 6)

    bands = {
        'f_center': np.round(f0, 3).tolist(),
        'f_bound': list(zip(flow.round(3).tolist(), fhigh.round(3).tolist())),
    }
    return bands
